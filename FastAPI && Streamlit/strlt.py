...
streamlit_image = Image.open('images/streamlit.png')
image_telecoms = Image.open('images/telecoms.jpg')
...
if add_selectbox == 'Online':
    ...
    st.success(
        "Les scénarios de prédiction en ligne sont pour les cas où vous voulez générer des prédictions sur une base individuelle")
    ...
    output = ""
    input_dict = {'state': state, 'account_length': account_length, 'area_code': area_code,
                  ...
                  'total_intl_charge': total_intl_charge, 'customer_service_calls': customer_service_calls}
...
if add_selectbox == 'Batch':
    image_telecoms2 = Image.open('images/telecom2.png')
    st.sidebar.image(image_telecoms2)
    st.sidebar.title("PROCESSING")

    add_selectbox2 = st.sidebar.selectbox("Datasciences", (
        "Choose...", "INFORMATIONS ON DATASET", 'VISUALISATION', 'DEPLOYMENT OF MODEL'))
    ...
    if add_selectbox2 == 'Choose...':
        ...
        lot_image = Image.open('images/lots.png')
        st.image(lot_image)
        st.success(
    ...
    if add_selectbox2 == 'VISUALISATION':
        ...
        st.header('1. LOADING OF CSV FILE🔧')
        file_upload = st.file_uploader("Please upload a csv file for visualization", type=["csv"])
            ...
            if chart == "scatter":
                ...
                if x and y:
                    ...
                    try:
                        ...
                        st.subheader('You can`\'t do that kind of visualization')
                        pass
